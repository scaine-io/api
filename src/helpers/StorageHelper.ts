...
export async function pipelineDownloadToGCS(
  url: string,
  storage: Storage,
  media: Media
) {
  try {
    ...
    throw new Error(
      `Error in pipelineDownloadToGCS: ${error.message} for ${url}`
    );
  }
}
...
export async function uploadBufferToGCS(
  ...
  try {
    ...
    const fileType = getContentTypeByExtension(fileName);

    await file.save(buffer, {
      resumable: false, // Prevents a potential "resumable" upload session, which is not needed for a buffer.
      metadata: {
        contentType: fileType, // Set the content type based on your file
      },
    });
  } catch (error: any) {
